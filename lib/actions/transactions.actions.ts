...
// Get transactions by bank ID from Appwrite database
export const getTransactionsByBankId = async ({ bankId }: getTransactionsByBankIdProps) => {
  try {
    const { database } = await createAdminClient();

    // Get transactions where the bank is either sender or receiver
...
// Get a specific transaction by its ID from Appwrite
